...
class ModulePackDownloadView(APIView):
    ...
    def get(self, request, pk):
        ...
        from .views import module_unlocked
        if not module_unlocked(request.user,module):
            return HttpResponse('Module is locked.', status=403, content_type='text/plain')
        buf=BytesIO(); c=canvas.Canvas(buf,pagesize=A4); width,height=A4; y=height-22*mm
        def line(txt,size=10,bold=False,gap=5):
            nonlocal y,c
            if y<20*mm: c.showPage(); y=height-20*mm
            c.setFont('Helvetica-Bold' if bold else 'Helvetica',size);
            for part in str(txt).split('\n'):
                c.drawString(18*mm,y,part[:115]); y-=gap*mm
        line(module.title,18,True,8); line(module.summary or '',10,False,5); line('Learning objectives',13,True,6)
        for obj in (module.objectives or []): line('• '+obj,10,False,5)
        line('Lessons',13,True,6)
        for lesson in module.lessons.filter(active=True).order_by('order'):
            line(f'Lesson {lesson.order}: {lesson.title}',12,True,6); line(lesson.body or '',9,False,4)
        quiz=getattr(module,'quiz',None)
        if quiz:
            line(f'Assessment — Pass {quiz.pass_mark}%',13,True,6)
            for i,q in enumerate(quiz.questions.all(),1):
                line(f'{i}. {q.prompt}',9,True,4)
                for j,ch in enumerate(q.choices.all(),1): line(f'   {chr(64+j)}. {ch.text}',9,False,4)
        line('RSRE Research Academy — learner module pack',8,False,4)
        c.save(); buf.seek(0)
        response=HttpResponse(buf.getvalue(),content_type='application/pdf')
        response['Content-Disposition']=f'attachment; filename="{module.slug}-module-pack.pdf"'
        return response
...
class CertificateDownloadView(APIView):
    ...
    def get(self, request, certificate_id):
        ...
        brand=certificate_settings(); buf=BytesIO(); c=canvas.Canvas(buf,pagesize=landscape(A4)); width,height=landscape(A4)
        ...
        c.drawRightString(width-25*mm,f'{brand.organization_name} · {timezone.localdate()}')
        c.save(); buf.seek(0); response=HttpResponse(buf.getvalue(),content_type='application/pdf'); response['Content-Disposition']=f'attachment; filename="{cert.certificate_id}.pdf"'; return response
