...
def public_settings(obj):
    ...
    return {
        ...
        **bank_details,
        'support_whatsapp_number': getattr(settings, 'SUPPORT_WHATSAPP_NUMBER', ''),
    }

def payment_data(obj, request=None):
    data = {
        ...
        'status': obj.status, 'created_at': obj.created_at,
    }
    viewer = getattr(request, 'user', None)
    if viewer and viewer.is_authenticated and (
        is_admin(viewer) or obj.payer_user_id == viewer.id
    ):
        data.update({
            'proof_url': obj.proof.url if obj.proof else '',
...
class PaymentSettingsView(APIView):
    ...
    def put(self, request):
        ...
        obj, _ = PaymentSettings.objects.get_or_create(singleton_key=1)
        editable_fields = {
            'organisation_name', 'support_message', 'currency',
            'mtn_enabled', 'mtn_display_number', 'airtel_enabled',
            'airtel_display_number', 'bank_enabled', 'bank_name',
...
class SupportPaymentCreateView(APIView):
    ...
    def post(self, request):
        ...
        obj = SupportPayment.objects.create(
            ...
            payer_name=payer_name, payer_email=(d.get('payer_email') or '').strip(),
            ...
            proof=request.FILES.get('proof'), status='pending'
        )
        if obj.payer_email:
        ...
        return Response(payment_data(obj, request), status=201)

class MySupportPaymentsView(APIView):
    permission_classes = [IsAuthenticated]
    def get(self, request):
        return Response([payment_data(x, request) for x in SupportPayment.objects.filter(payer_user=request.user)])

class AdminSupportPaymentsView(APIView):
    permission_classes = [IsAuthenticated]
    def get(self, request):
        if not is_admin(request.user): return Response({'detail':'Administrator access required.'}, status=403)
        return Response([payment_data(x, request) for x in SupportPayment.objects.all()[:200]])
    def patch(self, request, pk):
        ...
        return Response(payment_data(obj, request))
