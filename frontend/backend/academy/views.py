...
class SubmitQuizView(APIView):
    ...
    @transaction.atomic
    def post(self, request, pk):
        quiz = get_object_or_404(Quiz.objects.select_related("module", "module__level", "module__pathway").prefetch_related("questions__choices"), pk=pk)
        ...
        return Response({"score": float(score), "passed": passed, "pass_mark": quiz.pass_mark, "results": per_question})
...
class AIResearchCoachView(APIView):
    ...
    def post(self,request):
        ...
        templates={
            ...
            'explain':'Break the concept into: definition → why it matters → health-research example → common mistake → quick self-test.',
...
class AcademyAdminEnhancementsView(APIView):
    ...
    def post(self,request):
        ...
        if kind=='badge':
            obj=Badge.objects.create(name=request.data['name'],code=request.data.get('code',request.data['name'].lower().replace(' ','-')),description=request.data.get('description',''),icon=request.data.get('icon','🏅'))
