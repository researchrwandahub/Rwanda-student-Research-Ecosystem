    ...
    <main className="mx-auto max-w-7xl px-4 pb-16 sm:px-6">
      <section className="rounded-[2rem] bg-gradient-to-br from-slate-950 via-blue-950 to-indigo-900 p-7 text-white shadow-lg sm:p-9">
        <div className="flex flex-col gap-7 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="text-xs font-black uppercase tracking-[0.2em] text-blue-200">Research identity â€¢ evidence â€¢ progression</div>
      ...
      {loading && <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-8 text-slate-500">Loading your research recordâ€¦</div>}
      ...
      {data && <>
        ...
        {tab === 'evidence' && <section className="mt-7 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          <form onSubmit={addEvidence} className="rounded-3xl border border-slate-200 bg-white p-7 shadow-sm">
            ...
            <button disabled={saving} className="mt-4 rounded-xl bg-slate-950 px-5 py-3 text-sm font-black text-white">{saving ? 'Savingâ€¦' : 'Add declared evidence'}</button>
          </form>
          <section className="rounded-3xl border border-slate-200 bg-white p-7 shadow-sm">
            ...
            <div className="mt-5 space-y-3">{data.recent_evidence.length ? data.recent_evidence.map(ev => <div key={ev.id} className="rounded-2xl border border-slate-100 p-4"><div className="flex items-start justify-between gap-4"><div><div className="font-black text-slate-900">{ev.title}</div><div className="mt-1 text-xs font-bold uppercase tracking-wider text-slate-400">{ev.evidence_type} Â· {ev.source_type}</div></div><span className={`rounded-full px-2.5 py-1 text-[11px] font-black ${ev.source_type === 'manual' ? 'bg-amber-50 text-amber-700' : ev.source_type === 'verified' ? 'bg-emerald-50 text-emerald-700' : 'bg-blue-50 text-blue-700'}`}>{ev.source_type === 'manual' ? 'Declared' : ev.source_type === 'verified' ? 'Verified' : 'RSRE recorded'}</span></div><p className="mt-2 text-sm leading-6 text-slate-500">{ev.description}</p></div>) : <div className="rounded-2xl bg-slate-50 p-6 text-sm text-slate-500">Your RSRE activity will appear here as it is recorded.</div>}</div>
          </section>
        </section>}
