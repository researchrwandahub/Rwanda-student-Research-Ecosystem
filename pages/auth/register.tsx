...
export default function Register() {
  ...
  const [form, setForm] = useState({
    username: "", email: "", password: "", password2: "", role: "author", invitation_code: "",
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const registerData = {
        ...
        full_name: form.username, university: form.university, department: form.department,
      ...
      setMessage(typeof errors === "string" ? errors : JSON.stringify(errors || "Registration failed."));
    } finally {
      setLoading(false);
    }
  }
          ...
          <div className="mx-auto max-w-4xl rounded-[2rem] bg-white/95 backdrop-blur shadow-2xl p-7 md:p-10">
            ...
            <form onSubmit={handleSubmit} className="grid gap-6 mt-7">
              <div className="grid md:grid-cols-2 gap-5">
                ...
                <label className="grid gap-2 text-sm font-semibold text-slate-800">Email<input type="email" name="email" value={form.email} onChange={handleChange} required className="w-full rounded-xl border px-4 py-3 font-normal" /></label>
              </div>
