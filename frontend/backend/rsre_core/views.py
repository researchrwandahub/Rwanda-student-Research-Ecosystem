...
from .services import send_email_notification, send_whatsapp_notification, dispatch_notification, create_whatsapp_group
...
class EthicsAssessmentDetailView(APIView):
    ...
    def get_object(self, request, pk):
        return EthicsAssessment.objects.get(pk=pk, user=request.user)
...
class CollaborationNetworkView(APIView):
    ...
    def post(self, request):
        ...
        if not recipient_id or int(recipient_id) == request.user.id:
