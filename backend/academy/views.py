...
def evaluate_badges(user):
    ...
    for badge in rules:
        ...
        if eligible:
            ...
            awards.append(award)
            if created:
                notify_academy(user, f"Research Academy — badge earned: {badge.name}", badge.description or f"You earned the {badge.name} badge.", "/research-academy/dashboard", "View achievement")
    return awards
    ...
    for level in Level.objects.filter(active=True).order_by("number"):
        if level_completed(user, level):
            ...
            if created:
                notify_academy(
                    user,
                    f"Research Academy — {level.name} completed",
    ...
    for pathway in SpecialistPathway.objects.filter(active=True).order_by("id"):
        if pathway_completed(user, pathway):
            ...
            if created:
                notify_academy(
                    user,
                    f"Research Academy — {pathway.name} completed",
...
class EnrollmentView(APIView):
    ...
    def post(self, request):
        ...
        if created:
            notify_academy(request.user, "Welcome to the Research Academy", "Your Academy progress is now enabled. Start with Beginner — Foundations for Health Research.", "/research-academy", "Start learning")
...
class AcademyAdminCertificateRevokeView(APIView):
    permission_classes = [IsAuthenticated]

    def post(self, request, certificate_id):
        if not is_academy_admin(request.user):
            return Response({"detail": "Forbidden"}, status=403)
        cert = (
            ModuleCertificate.objects.filter(certificate_id=certificate_id).first()
            or LevelCertificate.objects.filter(certificate_id=certificate_id).first()
            or PathwayCertificate.objects.filter(certificate_id=certificate_id).first()
        )
        if cert is None:
            return Response({"detail": "Certificate not found."}, status=404)
        cert.status = "revoked"
        cert.save(update_fields=["status"])
        return Response({"certificate_id": cert.certificate_id, "status": cert.status})
...
class CourseListView(APIView):
    permission_classes = [AllowAny]
    def get(self, request):
        qs = AcademyCourse.objects.filter(active=True, status="published").select_related("level", "pathway").prefetch_related("modules")
...
class CourseDetailView(APIView):
    permission_classes = [AllowAny]
    def get(self, request, pk):
        preview = (request.query_params.get("preview") == "1" or request.path.endswith("/preview/")) and is_academy_admin(request.user)
        course_qs = AcademyCourse.objects.select_related("level", "pathway")
        if not preview:
            course_qs = course_qs.filter(active=True, status="published")
        course = get_object_or_404(course_qs, pk=pk)
...
class AcademyAdminCourseLifecycleView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def patch(self, request, pk):
        if not is_academy_admin(request.user):
            return Response({"detail": "Academy administrator access required."}, status=403)
        course = get_object_or_404(AcademyCourse, pk=pk)
        action = request.data.get("status", request.data.get("action"))
        try:
            if action in ("publish", "published"):
                ...
                course.status = "draft"
                course.active = True
                course.save(update_fields=["status", "active", "updated_at"])
            else:
                return Response({"detail": "status must be publish, draft, or archive."}, status=400)
        except ValidationError as exc:
            return Response({"detail": exc.message_dict if hasattr(exc, "message_dict") else exc.messages}, status=400)
        return Response({"id": course.id, "status": course.status, "active": course.active,
                         "published_at": course.published_at, "archived_at": course.archived_at})
...
class AcademyEnhancementsView(APIView):
    permission_classes = [AllowAny]
    def get(self, request):
        labs = list(PracticeLab.objects.filter(active=True).select_related('module').values('id','title','description','instructions','module_id','rubric','pass_mark','required','attempts_allowed'))
...
class AcademyAdminEnhancementsView(APIView):
    ...
    def post(self,request):
        ...
        if kind=='lab':
            obj=PracticeLab.objects.create(module_id=request.data.get('module_id') or None,title=request.data['title'],description=request.data.get('description',''),instructions=request.data.get('instructions',''),rubric=request.data.get('rubric',[]),pass_mark=int(request.data.get('pass_mark',80)),required=bool(request.data.get('required',False)),attempts_allowed=int(request.data.get('attempts_allowed',1)))
    ...
    def patch(self, request):
        if not admin_required(request):
            return Response({'detail':'Academy administrator access required.'}, status=403)
        if request.data.get('kind') == 'badge':
            obj = get_object_or_404(Badge, pk=request.data.get('id'))
            for field in ('name', 'code', 'description', 'icon', 'trigger_type', 'trigger_value', 'active'):
                if field in request.data:
                    setattr(obj, field, request.data[field])
            obj.save()
            return Response({'id': obj.id, 'name': obj.name, 'active': obj.active})
        return Response({'detail':'Unknown enhancement type.'}, status=400)
