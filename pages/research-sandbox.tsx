...
type Run = { id:number; title:string; method?:string; result_summary?:string; reproducibility_note?:string; status:string; created_at?:string };
type Workspace = { id:number; title:string; description:string; research_topic:string; visibility:string; status:string; notes:any[]; datasets:any[]; runs:Run[]; updated_at?:string }
...
export default function ResearchSandbox(){
  ...
  useEffect(()=>{load()},[])
  const create=async()=>{if(!title.trim())return; setBusy(true);setError(''); try{const r=await api.post('/research-sandbox/',{title:title.trim(), research_topic:topic.trim(), description:description.trim()}); setTitle('');setTopic('');setDescription(''); setWorkspaces(current=>[r.data,...current])}catch(e:any){setError(e?.response?.data?.detail||'Could not create workspace.')}finally{setBusy(false)}}
  return <ApplicationShell name="Research Sandbox" description="A private research workbench for experimentation, analysis and reproducibility." nav={[["/research-sandbox","Sandbox"],["/research-incubator","Incubator"],["/research-discovery","Discovery"],["/research-analytics","Analytics"]]}>
    <main className="mx-auto max-w-7xl px-6 pb-16 pt-6">
      ...
      <section className="mt-8 rounded-3xl border border-slate-200 bg-white p-7 shadow-sm">
        ...
        <button disabled={busy||!title.trim()} onClick={create} className="mt-4 rounded-xl bg-slate-950 px-5 py-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-50">{busy?'Creating…':'Open workspace'}</button>
      </section>
      ...
      <div className="mt-5 grid gap-5 md:grid-cols-2">{workspaces.map(w=><article key={w.id} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"><div className="flex items-start justify-between gap-4"><div><div className="text-xs font-black uppercase tracking-[0.16em] text-cyan-700">{w.visibility}</div><h4 className="mt-1 text-2xl font-black text-slate-950">{w.title}</h4></div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">{w.status}</span></div><p className="mt-3 text-sm leading-6 text-slate-600">{w.description || 'No purpose statement yet.'}</p><div className="mt-2 text-xs font-bold text-slate-500">Topic: {w.research_topic || 'Not specified'}</div><div className="mt-5 grid grid-cols-3 gap-3"><div className="rounded-2xl bg-slate-50 p-4"><div className="text-2xl font-black">{w.notes?.length||0}</div><div className="text-xs font-bold text-slate-500">Notes</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-2xl font-black">{w.datasets?.length||0}</div><div className="text-xs font-bold text-slate-500">Datasets</div></div><div className="rounded-2xl bg-slate-50 p-4"><div className="text-2xl font-black">{w.runs?.length||0}</div><div className="text-xs font-bold text-slate-500">Analysis runs</div></div></div><div className="mt-4 rounded-2xl border border-slate-100 bg-slate-50 p-4"><div className="text-xs font-black uppercase tracking-wide text-slate-400">Latest analysis</div><div className="mt-2 text-sm font-bold text-slate-700">{w.runs?.[0]?.title || 'No analysis run recorded yet.'}</div>{w.runs?.[0]?.result_summary && <p className="mt-1 text-xs leading-5 text-slate-500">{w.runs[0].result_summary}</p>}</div><Link href={`/research-sandbox/${w.id}`} className="mt-5 inline-flex rounded-xl border border-slate-200 px-4 py-2 text-sm font-black text-slate-900 hover:bg-slate-50">Open workbench →</Link></article>)}{!workspaces.length&&!error&&<div className="rounded-3xl border border-dashed border-slate-300 p-8 text-sm text-slate-500">Your first sandbox workspace will appear here.</div>}</div></section>
    </main>
  </ApplicationShell>
}
