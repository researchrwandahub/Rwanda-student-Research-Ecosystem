  ...
  return <footer className="mt-16 bg-[#071b34] text-white">
    <div className="rsjh-page grid gap-10 py-14 md:grid-cols-[1.35fr_1fr_1fr_1fr]">
      <div>
        ...
        <p className="mt-5 max-w-xl text-sm leading-7 text-blue-100">{SITE.tagline} A Rwanda-rooted research ecosystem connecting learning, discovery, opportunities, collaboration, publishing and evidence-to-action.</p>
      ...
      <div><h3 className="font-bold">Research & Publishing</h3><div className="mt-4 grid gap-3 text-sm text-blue-200"><Link href="/collaboration">Collaboration Network</Link><Link href="/ethics-compliance">Ethics & Compliance</Link><Link href="/events-training">Events & Training</Link><Link href="/articles">RSJH Journal</Link><Link href="/submit">Submit Manuscript</Link></div></div>
    ...
    <div className="border-t border-white/10"><div className="rsjh-page flex flex-col gap-2 py-5 text-xs text-blue-300 sm:flex-row sm:items-center sm:justify-between"><span>© {new Date().getFullYear()} {SITE.name}</span><span>Research integrity · Privacy · Human governance</span></div></div>
  </footer>
}
