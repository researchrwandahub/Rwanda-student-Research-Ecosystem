...
import { Bell, LayoutDashboard, Menu, Search, Sparkles, X } from 'lucide-react'
...
export default function Header() {
  ...
  return <header className="border-b border-slate-200/80 bg-white/95 backdrop-blur-xl">
    ...
    {open&&<div className="border-t border-slate-200 bg-white lg:hidden"><div className="rsre-page grid gap-1 py-3 text-sm font-bold">{primaryNav.concat(workspaceNav).map(([href,label])=><Link key={href} className="rsre-mobile-link" href={href} onClick={()=>setOpen(false)}>{label}</Link>)}{user?<><Link className="rsre-mobile-link" href={dashboard} onClick={()=>setOpen(false)}>Dashboard</Link><Link className="rsre-mobile-link" href="/notifications" onClick={()=>setOpen(false)}>Notifications</Link><Link className="rsre-mobile-link" href="/profile" onClick={()=>setOpen(false)}>Profile</Link><button className="rsre-mobile-link text-left text-rose-700" onClick={logout}>Logout</button></>:<Link className="rsre-mobile-link text-emerald-700" href="/auth/login" onClick={()=>setOpen(false)}>Sign in</Link>}</div></div>}
  </header>
}
