...
class AcademyCourse(models.Model):
    ...
    def publish_validation_errors(self):
        errors = {}
        if not self.title.strip():
            errors["title"] = "A course title is required."
        modules = list(self.modules.filter(active=True).order_by("order")) if self.pk else []
        if not modules:
            errors["modules"] = "At least one active module is required."
        elif any(not m.lessons.filter(active=True).exists() for m in modules):
            errors["lessons"] = "Every active module must contain at least one active lesson."
        return errors

    def clean(self):
        super().clean()
        if self.status == "published":
            errors = self.publish_validation_errors()
            if errors:
    ...
    def publish(self):
        ...
        self.save(update_fields=["status", "active", "published_at", "archived_at", "updated_at"])
        return self
    ...
    def archive(self):
        ...
        self.save(update_fields=["status", "active", "archived_at", "updated_at"])
        return self
