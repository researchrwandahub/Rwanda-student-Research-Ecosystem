...
class ReviewViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == "editor":
            return Review.objects.filter(article__handling_editor=user)
    ...
    def perform_update(self, serializer):
        user = self.request.user
        if not (is_administrator(user) or is_editorial_staff(user) or serializer.instance.reviewer == user):
            raise PermissionDenied("You can update only your own review.")
        serializer.save(reviewer=serializer.instance.reviewer)

    def perform_destroy(self, instance):
        user = self.request.user
        if not (is_administrator(user) or is_editorial_staff(user) or instance.reviewer == user):
...
class ReviewAssignmentViewSet(
    ...
    def perform_update(self, serializer):
        ...
        if user.role == "reviewer":
            # Reviewers may acknowledge/complete their assignment, never
            # redirect it to another manuscript or reviewer.
            serializer.save(article=assignment.article, reviewer=assignment.reviewer)
            return
        if not is_editorial_staff(user):
            raise PermissionDenied("Editorial access required.")
        serializer.save(article=assignment.article, reviewer=assignment.reviewer)
    ...
    def perform_destroy(self, instance):
        if not is_editorial_staff(self.request.user):
            raise PermissionDenied("Only editorial staff can remove assignments.")
        instance.delete()
...
class EditorialDecisionViewSet(viewsets.ModelViewSet):
    ...
    def _require_editorial_access(self):
        if not is_editorial_staff(self.request.user):
            raise PermissionDenied("Editorial access required.")

    def perform_update(self, serializer):
        self._require_editorial_access()
        serializer.save(editor=serializer.instance.editor)
...
class ResearchIdeaViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = ResearchIdea.objects.all().prefetch_related("projects")
        if is_administrator(self.request.user):
            return queryset
...
class ResearchProjectViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        user = self.request.user
        public = models.Q(visibility="public_summary")
        if not user.is_authenticated:
            return ResearchProject.objects.filter(public).select_related("owner", "mentor", "source_idea").prefetch_related("members__user", "milestones")
        if is_administrator(user) or getattr(user, "role", "") in {"editor", "editor_in_chief"}:
            return ResearchProject.objects.all().select_related("owner", "mentor", "source_idea").prefetch_related("members__user", "milestones")
        return ResearchProject.objects.filter(public | models.Q(owner=user) | models.Q(members__user=user, members__status="active")).distinct().select_related("owner", "mentor", "source_idea").prefetch_related("members__user", "milestones")

    def _can_manage(self, project):
        user = self.request.user
        return is_administrator(user) or project.owner_id == user.id

    def _can_edit(self, project):
        user = self.request.user
        return self._can_manage(project) or project.members.filter(user=user, status="active").exists()
    ...
    def perform_update(self, serializer):
        ...
        if not self._can_edit(project):
            raise PermissionDenied("Only an active project member, the owner or an administrator can update this project.")
        serializer.save()
    ...
    def add_member(self, request, pk=None):
        ...
        role = request.data.get("role", "co_investigator")
        if role not in dict(ResearchProjectMember.ROLE_CHOICES):
            return Response({"detail": "Invalid project member role."}, status=400)
        member, _ = ResearchProjectMember.objects.update_or_create(project=project, user=user, defaults={"role": role, "status": "active"})
    ...
    def remove_member(self, request, pk=None):
        ...
        if not self._can_manage(project):
            raise PermissionDenied("Only the project owner or an administrator can manage the team.")
        member = project.members.filter(pk=request.data.get("member_id")).first()
        if not member:
    ...
    def add_milestone(self, request, pk=None):
        project = self.get_object()
        if not self._can_edit(project): raise PermissionDenied("Only an active project member, the owner or an administrator can manage milestones.")
    ...
    def update_milestone(self, request, pk=None):
        project = self.get_object()
        if not self._can_edit(project):
            raise PermissionDenied("Only an active project member, the owner or an administrator can update milestones.")
        try:
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({
                ...
                "detail": "External opportunity source unavailable right now. Please try again later.",
            }, status=200)
...
class PassportEvidenceDetailView(APIView):
    ...
    def _get_owned(self, request, pk):
        return PassportEvidence.objects.filter(pk=pk, user=request.user, active=True).first()

    def put(self, request, pk):
        obj = self._get_owned(request, pk)
        if not obj:
            return Response({"detail": "Evidence not found."}, status=status.HTTP_404_NOT_FOUND)
        if obj.source_type != "manual":
            return Response({"detail": "Only manually added evidence can be edited."}, status=status.HTTP_403_FORBIDDEN)
        data = request.data.copy()
        for field in ("user", "source_type", "source_model", "source_object_id", "verification_note", "verification_code", "verified_at", "active"):
            data.pop(field, None)
        serializer = PassportEvidenceSerializer(obj, data=data, partial=True)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(PassportEvidenceSerializer(obj).data)
