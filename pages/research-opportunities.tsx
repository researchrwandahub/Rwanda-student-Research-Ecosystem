    ...
    >
      <main className="mx-auto max-w-7xl px-6 pb-16">
        ...
        <section className="mt-6 rounded-3xl border border-slate-200 bg-white p-7 shadow-sm">
          <div className="text-xs font-black uppercase tracking-[0.18em] text-slate-500">More places to search</div>
          <h2 className="mt-2 text-2xl font-black text-slate-950">Opportunity sources used by young researchers</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">These are external websites you can search directly for scholarships, fellowships, internships, grants, conferences and related programmes. RSRE provides this list as a discovery aid; a listing here is not an endorsement, guarantee of eligibility, or proof that an opportunity is current. Always verify the call on the official organiser website before applying or paying anything.</p>
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{[
            ['Opportunity Desk','https://opportunitydesk.org/'], ['Opportunities for Youth','https://opportunitiesforyouth.org/'], ['Opportunities For Africans','https://www.opportunitiesforafricans.com/'], ['Opportunity Tracker','https://opportunitytracker.ug/'], ['Opportunities Corners','https://opportunitiescorners.com/'], ['Opportunity Circle','https://www.opportunitiescircle.com/'], ['Global South Opportunities','https://www.globalsouthopportunities.com/'], ['Youth Opportunities','https://www.youthop.com/'], ['ProFellow','https://www.profellow.com/'], ['Scholars4Dev','https://www.scholars4dev.com/'], ['After School Africa','https://www.afterschoolafrica.com/'], ['FundsforNGOs','https://www2.fundsforngos.org/'], ['Studyportals','https://studyportals.com/'], ['ScholarshipPortal','https://www.scholarshipportal.com/'], ['FindAMasters','https://www.findamasters.com/'], ['IEFA','https://www.iefa.org/']
          ].map(([name,url])=><a key={name} href={url} target="_blank" rel="noopener noreferrer" className="rounded-2xl border border-slate-200 p-4 transition hover:border-emerald-300 hover:bg-slate-50"><div className="font-black text-slate-900">{name}</div><div className="mt-1 break-all text-xs text-slate-400">{url.replace(/^https?:\/\//,'')}</div><div className="mt-3 text-sm font-black text-emerald-700">Visit source â†—</div></a>)}</div>
          <div className="mt-5 rounded-2xl bg-slate-950 p-5 text-sm leading-7 text-slate-200"><div className="text-xs font-black uppercase tracking-[0.18em] text-emerald-300">For every young person who wants to go further</div><p className="mt-3 text-base font-black text-white">Opportunities are out there, but you have to search for them.</p><p className="mt-2">Scholarships, fellowships, internships, competitions, conferences, grants and leadership programmes appear throughout the year. Search, prepare and apply. Read the eligibility criteria, check the deadline, tailor your documents and submit through the organiserâ€™s official channel. You will face rejection; apply again.</p><p className="mt-2 font-bold text-white">Never pay random people who promise guaranteed selection. Always verify an opportunity through the official organiser website.</p><p className="mt-2 font-black text-emerald-300">Search. Prepare. Apply. Repeat.</p></div>
        </section>
        ...
        <section className="mt-6 flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:flex-row lg:items-center lg:justify-between">
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search grants, fellowships, internshipsâ€¦" className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none lg:max-w-md" />
